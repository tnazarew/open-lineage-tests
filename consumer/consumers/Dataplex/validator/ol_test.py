...
def match(expected, result) -> (bool, list):
    """
    ...
    if isinstance(expected, dict):
        ...
        for k, v in expected.items():
            if k not in result:
                return False, [f"Key {k} not in received event {result}\nExpected {expected}"]
            matched, errors = match(v, result[k])
            if not matched:
                errors.append([f"For key {k}, expected value {v} not equals received {result[k]}" + f"\nExpected {expected}, request {result}"])
                return False, errors
    elif isinstance(expected, list):
        if len(expected) != len(result):
            return False, [f"Length does not match: expected {len(expected)} result: {len(result)}"]
                ...
                for y in result:
                    if "name" in y:
                        ...
                        if expected_name == result_name:
                            matched, errors = match(x, y)
                            if not matched:
                                errors.append([f"List not matched {x} where {y}"])
                                return False, errors
                    matched = True
                    break
                if not matched:
                    return False, [f"List not matched - no same stuff for {x} " + f"in expected: {expected} result {result}"]
            else:
                matched, errors = match(x, result[i])
                if not matched:
                    errors.append([f"List not matched at {i}\n" + f"  expected:\n{json.dumps(x)}\n" + f"  result: \n{json.dumps(result[i])}"])
                    return False, errors
        ...
        if "{{" in expected:
            ...
            if rendered == "true" or rendered == result:
                return True, []
            return False, [f"Rendered value {rendered} does not equal 'true' or {result}"]
        elif expected != result:
            return False, [f"Expected value {expected} does not equal result {result}"]
    elif expected != result:
        return False, [f"Object of type {type(expected)}: {expected} does not match {result}"]
    return True, []
